...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "  --no-optimize       With --enforce or --compile: skip optimization passes"
            echo "                      (canonicalization only). Use to A/B optimized vs unoptimized."
            echo ""
            echo "  --compile <file>    Dry run: compile a profile JSON and report the emitted"
            echo "                      policy WITHOUT enforcing (no root/LSM needed). Great for"
            echo "                      showing rule-count shrink from the optimization passes."
            echo ""
            echo "  --emit-apparmor <file>  With --compile/--enforce: also write an AppArmor profile"
            echo "  --emit-json <file>      With --compile/--enforce: also write the optimized IR JSON"
            echo ""
...
# Resolve profile / emit paths against the caller's CWD *now*, before we cd into
# the crate directories below. The userspace binary ultimately runs from
# ebpf-mon/, so a bare relative path like "ebpf-mon/final-events.json" would
# otherwise be looked up from the wrong place and fail with "No such file...".
[ -n "$ENFORCE_PROFILE" ] && ENFORCE_PROFILE="$(realpath -m "$ENFORCE_PROFILE")"
[ -n "$COMPILE_PROFILE" ] && COMPILE_PROFILE="$(realpath -m "$COMPILE_PROFILE")"
[ -n "$EMIT_APPARMOR" ]   && EMIT_APPARMOR="$(realpath -m "$EMIT_APPARMOR")"
[ -n "$EMIT_JSON" ]       && EMIT_JSON="$(realpath -m "$EMIT_JSON")"

# Fail fast with a clear message if an input profile is missing/unreadable,
# instead of the cryptic "Error: ... No such file or directory" from the binary.
if [ -n "$ENFORCE_PROFILE" ] && [ ! -r "$ENFORCE_PROFILE" ]; then
    echo -e "${RED}[ERROR]${NC} Enforcement profile not found or unreadable: $ENFORCE_PROFILE"
    exit 1
fi
if [ -n "$COMPILE_PROFILE" ] && [ ! -r "$COMPILE_PROFILE" ]; then
    echo -e "${RED}[ERROR]${NC} Compile profile not found or unreadable: $COMPILE_PROFILE"
    exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Compile-only dry run: build eBPF bytecode (needed because the userspace binary
# embeds it via include_bytes!), then run --compile. No container, no sudo, no LSM.
if [ -n "$COMPILE_PROFILE" ]; then
    if [ -n "$ENFORCE_PROFILE" ]; then
        echo -e "${RED}[ERROR]${NC} --compile and --enforce are mutually exclusive"
        exit 1
    fi
    echo -e "${BLUE}[STEP 1/2]${NC} Building eBPF bytecode..."
    if ! (cd "$SCRIPT_DIR/ebpf-mon-ebpf" && cargo build --release 2>&1 | tail -5); then
        echo -e "${RED}[ERROR]${NC} Failed to build eBPF bytecode"
        exit 1
    fi
    EMIT_FLAGS=""
    [ -n "$EMIT_APPARMOR" ] && EMIT_FLAGS="$EMIT_FLAGS --emit-apparmor $EMIT_APPARMOR"
    [ -n "$EMIT_JSON" ] && EMIT_FLAGS="$EMIT_FLAGS --emit-json $EMIT_JSON"
    echo -e "${BLUE}[STEP 2/2]${NC} Compiling policy (dry run): ${GREEN}$COMPILE_PROFILE${NC}"
    [ -n "$NO_OPTIMIZE" ] && echo -e "${YELLOW}[INFO]${NC} Optimization: DISABLED (--no-optimize)"
    cd "$SCRIPT_DIR/ebpf-mon"
    RUST_LOG=info cargo run --release -- --compile "$COMPILE_PROFILE" $NO_OPTIMIZE $EMIT_FLAGS
    exit $?
fi
...
if [ -n "$CONTAINER_NAME" ]; then
    ...
    echo -e "${BLUE}[INFO]${NC} Validating container exists..."
    # Capture without letting `set -e` abort before the error handler below runs.
    DOCKER_EXIT=0
    DOCKER_OUTPUT=$(docker inspect "$CONTAINER_ID" -f '{{.Id}}' 2>&1) || DOCKER_EXIT=$?
...
ENFORCE_FLAGS=""
if [ -n "$ENFORCE_PROFILE" ]; then
    ENFORCE_FLAGS="--enforce $ENFORCE_PROFILE $AUDIT_ONLY $NO_OPTIMIZE"
    ...
    if [ -n "$NO_OPTIMIZE" ]; then
        echo -e "${YELLOW}[INFO]${NC} Optimization: DISABLED (--no-optimize; canonicalization only)"
    fi
elif [ -n "$NO_OPTIMIZE" ]; then
    echo -e "${RED}[ERROR]${NC} --no-optimize requires --enforce or --compile"
    exit 1
fi

# Run with proper cargo command.
# Use `sudo env RUST_LOG=info` (not `sudo -E`) so logging survives sudoers
# policies that ignore -E — otherwise ENFORCEMENT DENY/AUDIT lines are silent.
RUST_LOG=info cargo run --release \
    --config 'target."cfg(all())".runner="sudo env RUST_LOG=info"' \
