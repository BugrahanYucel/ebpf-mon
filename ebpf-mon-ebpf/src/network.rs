...
use ebpf_mon_common::policy::NetPolicyKey;

use crate::enforcement::{NET_CONNECT_POLICY, POLICY_CGROUPS, SKB_ENFORCE_CGID};
...
pub fn cgroup_skb_egress(ctx: SkBuffContext) -> i32 {
    // Fail open: on any error, pass the packet (1) rather than drop (0).
    // This program is monitor-only and must never affect traffic.
    process_skb(ctx, Direction::Egress, false).unwrap_or(1)
}
...
pub fn cgroup_skb_ingress(ctx: SkBuffContext) -> i32 {
    // Fail open: on any error, pass the packet (1) rather than drop (0).
    // This program is monitor-only and must never affect traffic.
    process_skb(ctx, Direction::Ingress, false).unwrap_or(1)
}

/// Enforcing egress twin of `cgroup_skb_egress`: same packet inspection and
/// event piping, but returns the network verdict (0 = drop) at connection-init
/// time instead of always passing. Attached (in place of the monitor egress)
/// only in `--enforce` mode. Still fails OPEN on any parse error so a bug can
/// never black-hole a container's traffic.
#[cgroup_skb]
pub fn cgroup_skb_egress_enforce(ctx: SkBuffContext) -> i32 {
    process_skb(ctx, Direction::Egress, true).unwrap_or(1)
}

/// Network verdict for the enforcing cgroup_skb program. Mirrors
/// `enforce_socket_connect`: allow if (cgid,dst_ip,dst_port) or the
/// wildcard-port entry is present; otherwise default-deny (drop), honoring
/// audit-only. Fails open if the cgroup isn't under enforcement.
#[inline(always)]
unsafe fn skb_egress_verdict(dst_ip: u32, dst_port: u32) -> i32 {
    let cgid = match SKB_ENFORCE_CGID.get(0) {
        Some(v) => *v,
        None => return 1,
    };
    let config = match POLICY_CGROUPS.get(&cgid) {
        Some(c) => *c,
        None => return 1, // cgroup not enforced -> pass
    };

    // A matched allowlist entry => allow. We DEREFERENCE the looked-up value
    // (`let v = *verdict`) and use it, exactly like the proven `enforce_socket_
    // connect` LSM twin. That matters for the verifier: a bare `.is_some()` lets
    // LLVM fold the maybe-null map pointer into a branchless OR with the `config`
    // bytes (`r |= map_ptr`), which stricter kernels (7.0.0-generic) reject as
    // "pointer arithmetic on map_value_or_null prohibited". Consuming the deref'd
    // value forces a genuine null-checked load inside the `if let Some` scope, so
    // the pointer never leaks into arithmetic.
    let key = NetPolicyKey { cgroup_id: cgid, dst_ip, dst_port, protocol: 0, _pad: [0u8; 7] };
    if let Some(verdict) = NET_CONNECT_POLICY.get(&key) {
        let v = *verdict;
        // v == 0 marks an explicit deny entry; enforce it (drop) unless audit-only.
        if v == 0 && config.audit_only == 0 {
            return 0;
        }
        return 1;
    }
    let wildcard = NetPolicyKey { cgroup_id: cgid, dst_ip, dst_port: 0, protocol: 0, _pad: [0u8; 7] };
    if let Some(verdict) = NET_CONNECT_POLICY.get(&wildcard) {
        let v = *verdict;
        if v == 0 && config.audit_only == 0 {
            return 0;
        }
    ...
    // No rule matched — default deny (allowlist model).
    if config.default_action == 0 && config.audit_only == 0 {
        return 0; // DROP
    }
    1
}
...
#[inline(always)]
pub fn process_skb(ctx: SkBuffContext, direction: Direction, enforce: bool) -> Result<i32, i64> {
    match alloc::init() {
        Ok(_) => (),
        Err(e) => {
            return Err(e as i64);
    ...
    // Decide the verdict at connection-initiation time (only SYN / new-UDP-flow
    // packets reach here; established traffic already passed above, so we never
    // break an allowed flow mid-stream). Compute BEFORE piping so a repeated
    // connection to a blocked destination is still dropped even after dedup.
    let verdict = if enforce && direction == Direction::Egress {
        unsafe { skb_egress_verdict(event.payload.dst_ip, event.payload.dst_port) }
    } else {
        1
    };
