...
use ebpf_mon_common::{
    ...
    path::MAX_PATH_DEPTH,
    policy::{
        ACCESS_READ, ACCESS_WRITE, NetPolicyKey, O_ACCMODE, PathPolicyKey, PatternKey,
        PolicyConfig, PrefixEntry,
    },
};

/// Requested access mode of an open, derived from `file->f_flags & O_ACCMODE`.
/// Returns a bitmask of `ACCESS_READ` / `ACCESS_WRITE`. Opens that request
/// neither (should not happen for regular files) are treated as a read so we
/// never accidentally fail open on a write.
#[inline(always)]
unsafe fn requested_access(file: &co_re::file) -> u8 {
    let flags = file.f_flags().unwrap_or(0);
    match flags & O_ACCMODE {
        0 => ACCESS_READ,                // O_RDONLY
        1 => ACCESS_WRITE,               // O_WRONLY
        _ => ACCESS_READ | ACCESS_WRITE, // O_RDWR (2) — needs both
    }
}

#[map]
pub static POLICY_CGROUPS: HashMap<u64, PolicyConfig> = HashMap::with_max_entries(64, 0);

/// Single-slot map holding the cgroup id the enforcing `cgroup_skb` program is
/// guarding. cgroup_skb runs in softirq context where reading the current
/// task's cgroup is unreliable, but the program is attached to exactly one
/// container cgroup, so the loader just publishes that id here for the network
/// verdict lookup. Index 0 only.
#[map]
pub static SKB_ENFORCE_CGID: aya_ebpf::maps::Array<u64> =
...
pub fn enforce_file_open(ctx: *mut ::core::ffi::c_void) -> i32 {
    ...
    // Tier 1: Exact path match via hash. The stored value is an access-mode
    // MASK (ACCESS_READ / ACCESS_WRITE), NOT a bare verdict — this is where
    // BPF-LSM stops being open-granularity: a write to a read-only entry is
    // denied, and a value of 0 is an explicit deny.
    let path_key = PathPolicyKey { cgroup_id: cgid, path_hash };
    if let Some(mask) = unsafe { FILE_PATH_POLICY.get(&path_key) } {
        let allow_mask = *mask;
        let req = unsafe { requested_access(&file) };
        if allow_mask != 0 && (allow_mask & req) == req {
            return 0; // every requested access mode is permitted
        }
        // Denied: explicit deny (mask==0) or an access mode that wasn't
        // profiled (e.g. a write to a file only ever observed being read).
        // pattern=Regular(0) marks a matched-but-mode-denied entry, which the
        // audit log distinguishes from a not-in-list default deny (pattern=255).
        emit_audit(&ctx, cgid, path_hash, PathPattern::Regular as u8, 0, 0, Some(path_buf));
...
#[map]
pub static NET_CONNECT_POLICY: HashMap<NetPolicyKey, u8> = HashMap::with_max_entries(4096, 0);
