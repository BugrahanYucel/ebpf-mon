...
/// Fast pre-resolution dedup identity for read/write. Keyed on the kernel inode
/// object address (a stable, per-filesystem-unique identity — `i_ino` alone can
/// collide across mounts) plus the operation, so a file read/written repeatedly
/// is recognized *before* the expensive path walk.
#[repr(C)]
#[derive(Clone, Copy)]
pub struct InodeDedupKey {
    pub inode: u64, // address of struct inode (kernel object identity)
    pub r_w: u8,    // 0 = read, 1 = write (same inode, different op stays distinct)
    pub _padding: [u8; 7],
}

/// Pre-resolution dedup for the vfs read/write fexit hot path. LRU so it
/// self-evicts under churn. See `process_vfs_fexit` for why this exists.
#[map]
static INODE_DEDUP: LruHashMap<InodeDedupKey, u8> = LruHashMap::with_max_entries(65536, 0);
...
fn process_vfs_fexit(ctx: FExitContext, f_op: FileOperation) -> Result<u32, u32> {
    ...
    // ── Pre-resolution dedup (the hot-path optimization) ─────────────────────
    // core_resolve_file (full dentry walk) + classify_path dominate this hook's
    // cost, but for a file accessed repeatedly the resulting event is ultimately
    // dropped by the path-based dedup below. Short-circuit here, keyed on the
    // kernel inode identity + operation, so repeat accesses skip resolution,
    // classification, and the large allocations entirely.
    //
    // Skipped when a symlink was followed this syscall: that path needs the full
    // sym_path capture, and it is rare, so it always takes the slow path.
    // The path-based dedup remains the authoritative stage for first sightings.
    //
    // Trade-off: this coarsens read/write dedup to per-inode. Distinct paths that
    // share an inode (hardlinks/bind mounts) collapse to one event per operation
    // — intended for read/write noise reduction; opens still dedup per exact path.
    let r_w = if f_op == FileOperation::WRITE { 1u8 } else { 0u8 };
    let symlink_pending = unsafe { SYMLINK_MAP.get(pid_tgid).is_some() };
    if !symlink_pending {
        if let Some(inode_ptr) = unsafe { file.f_inode() } {
            let key = InodeDedupKey { inode: inode_ptr.as_ptr() as u64, r_w, _padding: [0u8; 7] };
            // First sighting -> Ok (fall through, emit). Repeat -> Err (bail early).
            if INODE_DEDUP.insert(&key, &1u8, BPF_NOEXIST.into()).is_err() {
                return Ok(0u32);
            }
        }
    }
