...
pub async fn listen_all_events(
    ...
    let cpu_ids = online_cpus().unwrap();

    // Open every per-CPU perf buffer FIRST. `perf_array.open` installs the perf
    // event fd into the PERF_EVENT_ARRAY map slot for that CPU. Until that slot
    // is populated, an in-kernel `bpf_perf_event_output(..., BPF_F_CURRENT_CPU)`
    // returns -ENOENT and the event is dropped silently (it is NOT counted as a
    // "lost" ring-buffer event). We therefore open all buffers, then signal the
    // caller so it can attach the probes only once a reader exists everywhere.
    let mut opened = Vec::with_capacity(cpu_ids.len());
    for cpu_id in cpu_ids {
        match perf_array.open(cpu_id, Some(128)) {
            Ok(buf) => opened.push((cpu_id, buf)),
            Err(e) => error!("Failed to open perf buffer for cpu {}: {}", cpu_id, e),
        }
    }

    // All buffers are installed; unblock probe attachment.
    let _ = ready.send(());

    for (cpu_id, mut buf) in opened {
        let tx_clone = tx_event.clone();

        debug!("Started event listener for CPU {}", cpu_id);
            ...
            loop {
                ...
                match events {
                    Ok(events) => {
                        ...
                        if events.lost > 0 {
                            warn!("cpu {} lost {} events (read {})", cpu_id, events.lost, events.read);
                        }

                        if events.read > 0 {
                            total_read += events.read as u64;
                            debug!("cpu {} read {} (total {}) lost {}", cpu_id, events.read, total_read, events.lost);
                        } else {
