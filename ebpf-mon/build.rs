...
//         .into_iter()
//         .find(|cargo_metadata::Package { name, .. }| name == "ebpf-mon-ebpf")
//         .ok_or_else(|| anyhow!("ebpf-mon-ebpf package not found"))?;
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "ebpf-mon-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `ebpf-mon-ebpf`, so cargo will automatically rebuild us
        // if `ebpf-mon-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `ebpf-mon-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("ebpf-mon-ebpf");
